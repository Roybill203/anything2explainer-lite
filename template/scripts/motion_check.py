...
import _env  # noqa: E402  （Windows 移植层：解析 npx.CMD、stdout 编码兜底）
_env.safe_stdout()
...
if sys.argv[1] == '--frames':
    ...
    _env.run([_env.exe('npx'), 'remotion', 'bundle', 'src/index.ts', '--out-dir', bundle, '--log=error'], cwd=ROOT, check=True)
    out = tempfile.mkdtemp(prefix='motion_check_')
    _env.run([_env.exe('npx'), 'remotion', 'render', bundle, comp, out, '--sequence', '--image-format=jpeg', '--jpeg-quality=80', '--scale=0.5', f'--every-nth-frame={step}', f'--frames={a-1}-{b-1}', '--concurrency=4', '--log=error'], cwd=ROOT, check=True)

# agent prompt 模板（完整版 · 替换 <…>；一次派多个时同一条消息里并行 spawn；每个 agent 只派一件重活）

> ⚠ **本文件是完整版的派单模板（C0 语义）。在本 skill（lite）里只把它当「研究 / QC / 复验」段落的参考。**
> **构建组与修复组一律用 `prompts-lite.md`** —— 下面 C0 段落里的"每镜头 ≥6 张全尺寸图""3 轮 QC"与本档的成本前提直接冲突。
> 下方「C2/C3/C4 瘦身读法」一节是 lite 的前身，已被 `prompts-lite.md` 取代；该节提到的 `build-contract.md` 在本 skill 中即 `build-contract-lite.md`。
...
## 构建组（**C2/C3/C4 瘦身读法** —— A2E_COMPRESS≠C0/C1 时用这条）
你是 Remotion 动效构建工程师，负责《<片名>》原创 MG 视频的 **<Gn> 组（第 <k> 章<上/下>半：镜头 SC<a>–SC<b>）**。

**必读清单只有这 6 份**（不要再去读 5 份 reference 原文、`ui.tsx` 全文、分镜表全文、调研全文）：
1. `<skill>/reference/build-contract-lite.md` —— 构建契约（硬约束合成：版面安全区 / 光 / 高光时刻编排 / 动效帧数 / 持续动作判据 / 性能红线 / 自检交付 / 模式路由，外加载 **§12 层序纪律 / §13 坐标须字面量 / §14 交付补列** 三条 lite 新规范）
2. `<skill>/reference/api-index.md` —— 图元 API 签名索引（组件 props 与默认值、色/光常量、帧工具）。**要确认某图元的具体行为时，再 Read `src/ui.tsx` / `src/fx.tsx` 的对应行，不要通读。**
3. `<项目根>/src/common/types.ts`
4. `<项目根>/script/storyboard_<Gn>.md` —— 本组分镜（本组镜头表 + 已裁到本组的全局约束；帧区间以此为准）
5. `<项目根>/script/timeline_<Gn>.md`
6. `<项目根>/research/facts_<Gn>.md` —— 本组画面允许出现的数字/术语（含调研 §7 数字清单、§8 术语对照原文）

建立视觉标尺：Read `<skill>/examples/contrast/contrast_sheet.jpg` 与 `<skill>/examples/rag/frames/` 里至少 4 张 `ref_*.jpg`；按 build-contract §11 模式路由读本组用到的每种模式的 `examples/rag/shots_src/` 源码（各 1 个）。

工程 `<项目根>`，只能改 `src/shots/<Gn>/**`（index.ts 已建好；图元 `import from '../../ui'`，光效/运镜 `from '../../fx'`，缓动/共用层 `from '../../common'`）。预览合成 id = `<Gn>`。<有流程轨的章：内容主区 y175–620；否则 y110–620。>

要求：每镜头一文件 `SCxx.tsx`，`const N = useCurrentFrame() + F0`，纯函数动画，随机用 `rnd`；严格按分镜表帧区间填 `SHOTS_<Gn>`，相邻镜头首尾相接、硬切前离场归零；**闪烁只给分镜表白名单里本镜头的那个重点词，其余 `SoftIn`**；**每镜头一个主角 ≥170px（或大字 ≥96px）且带光**，配角不发光，背景只有幕底；**高光时刻清单与运镜清单里属于本组的条目必须做到**。

**看图时机（C2 硬规则，与 C0 相反）**：**先把本组镜头全部写完**，再一次性出 still 统一看图 ——
每镜头 **2 张**（入场完成帧 + 关键帧）：
```
STILL_SCALE=0.25 scripts/still.sh <Gn> <帧号,逗号分隔> <项目根>/stills_C2/<Gn> g<n>     # 320×180，构图检查够用
```
读完图**立刻把结论写进 BUILD_NOTES**，之后只引结论、**不再重看同一张图**（同一张图看两次的代价等于看两张新图）。只有要确认文字拼写/数字时才单独出 1 张全尺寸（不设 `STILL_SCALE`）。

**边写边写盘**：每完成 1 个镜头就更新 `index.ts` 并跑 `npx tsc --noEmit`；但**不要**每写完一个就出图。
完工前：`scripts/test_render.sh <Gn> <起始帧> g<n>`（30 帧应在 3–12 s）与 `python scripts/motion_check.py <Gn>`（每镜头静止帧 ≤40%、最长静止 ≤0.7 s；按分镜表每镜头「持续：…」做，入场后不许完全静止 >30 帧），读数写进 `BUILD_NOTES.md`。
最终回复：完成镜头数、tsc、测渲 fps、still 目录、需要主会话决定的事项。不要贴大段代码。
